...
def append_VariableData_to_netcdf(nc_out_fname, var, var_data, fv=None, log=True, indent=''):
    ...
    sprint(_n, ' Now try to append variable <{0}>'.format(meta.get_name()), log=log, mode='bold', indent=indent)
    # --------------------------------------------------
    #                   Appending ncdf
    # --------------------------------------------------
    ...
    if meta.get_name() in nc.variables.keys():
        sprint(' Variable <{0}> skipped. Already exists in file <{1}>'.format(meta.get_name(), nc_out_fname), log=log, mode='warning', indent=_i)
        nc.close()
        return
    ...
    if var_data is not None:
        ...
        if nc_var_shape != var_data.shape:
            ...
            if len(nc_var_shape) != len(var_data.shape):
                msg = 'Invalid dimensions. Passed metadata {0} declares following dimension names {1} for variable {2}. NetCDF file {3} has following lengths of previously mentioned dimensions {4}. Therefore exacly this shape {4} will be allocated for an array within NetCDF file. However passed data-array has shape {5} and cannot be fit into allocated shape'.format(type(var), meta.get_dims(), meta.get_name(), nc_out_fname, nc_var_shape, var_data.shape)
                msg2 = 'Now I will squeeze (see numpy.squeeze()) passed data-array and try to fit it into allocated array within NetCDF file once again'
                if log:
                    msg_printed = True
                    sprint(msg, log=log, mode='warning', indent=_i)
                    sprint(msg2, log=log, indent=_i)
                ...
                if len(squeezed_ncvarshape) != len(squeezed_vdatashape):
                    if not msg_printed:
                        sprint(msg, log=log, mode='warning', indent=_i)
                        sprint(msg2, log=log, indent=_i)
                    sprint('Squeezed arrays do not match each other (allocated squeezed shape {0} != data-array squeezed shape {1}). Proceeding...'.format(squeezed_ncvarshape, squeezed_vdatashape), log=log, indent=_i, mode='warning')
                    ui.promt(_n+' Press Enter to skip appending variable <{0}>'.format(meta.get_name()), color='yellow', pause=True)
                    nc.close()
                    return
                else:
                    sprint('Squeezed arrays match each other (allocated squeezed shape {0} = data-array squeezed shape {1}). Proceeding...'.format(squeezed_ncvarshape, squeezed_vdatashape), log=log, indent=_i)
                ...
                for i, dim_length_nc, dim_length_data in zip(xrange(len(nc_var_shape)), nc_var_shape, var_data.shape):
                    if (dim_length_nc != dim_length_data and dim_length_nc == 0 and nc.dimensions[meta.get_dims()[i]].isunlimited()):
                        ...
                        sprint(msg, mode='fail', indent=_i)
                        ui.promt(_n+' Press Enter to skip appending variable <{0}>'.format(meta.get_name()), color='yellow', pause=True)
                        nc.close()
                        return
    ...
    if meta.get_attrs():
        for attr_name, attr_value in meta.get_attrs().iteritems():
            if not attr_name.startswith('_'):  # reserved for special attributes
                ...
                sprint('add attribute <{0} = {1}> of type {2}'.format(attr_name, attr_value, type(attr_value)), log=log, indent=_i)
    ...
    sprint('Data has shape:', nc_var.shape, log=log, indent=_i)
    sprint('Variable appended successfully: ', meta.get_name(), log=log, mode='ok', indent=_i)
    nc.close()
